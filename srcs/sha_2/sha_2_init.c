...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/03 21:40:56 by vsteffen          #+#    #+#             */
/*   Updated: 2019/06/03 21:40:57 by vsteffen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (sha_2->type == 0)
		ft_memcpy(digest, (unsigned int[]){0xc1059ed8, 0x367cd507, 0x3070dd17,
			0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4},
			sizeof(uint32_t) * 8);
	else if (sha_2->type == 1)
		ft_memcpy(digest, (unsigned int[]){0x6a09e667, 0xbb67ae85, 0x3c6ef372,
			0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19},
			sizeof(uint32_t) * 8);
	else if (sha_2->type == 2)
		ft_memcpy(digest, (unsigned long long[]){0xcbbb9d5dc1059ed8,
			0x629a292a367cd507, 0x9159015a3070dd17, 0x152fecd8f70e5939,
			0x67332667ffc00b31, 0x8eb44a8768581511, 0xdb0c2e0d64f98fa7,
			0x47b5481dbefa4fa4}, sizeof(uint64_t) * 8);
	else
		ft_memcpy(digest, (unsigned long long[]){0x6a09e667f3bcc908,
			0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1,
			0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b,
			0x5be0cd19137e2179}, sizeof(uint64_t) * 8);
}

void		sha_2_init_k_64(uint64_t *k)
{
...
void		sha_2_init_struct_64(t_sha_2 *sha_2, t_ssl *ssl)
{
...
void		sha_2_init_struct(t_sha_2 *sha_2, t_ssl *ssl, t_ssl_in **cur_input,
	int8_t *ret)
{
	sha_2->ssl = ssl;
	...
	*ret = 1;
	if (ft_strcmp("sha224", ssl->crypt->name) == 0)
	{
		...
		sha_2->padding_length = SSL_PADDING_LENGTH_SHA_32;
	}
